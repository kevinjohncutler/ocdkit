"""Tests for ocdkit.plot.grid — image_grid_matplotlib layout and rendering."""

import numpy as np
from matplotlib.figure import Figure

from ocdkit.plot.grid import image_grid_matplotlib
...
class TestImageGrid:
    ...
    def test_basic_grid(self):
        ...
        fig = image_grid_matplotlib(imgs)
    ...
    def test_column_titles(self):
        ...
        fig = image_grid_matplotlib(imgs, column_titles=['A', 'B'])
        assert isinstance(fig, Figure)

    def test_row_titles(self):
        imgs = self._rand_imgs(2, 2)
        fig = image_grid_matplotlib(imgs, row_titles=['Row1', 'Row2'])
        assert isinstance(fig, Figure)

    def test_plot_labels(self):
        imgs = self._rand_imgs(1, 3)
        labels = [['a', 'b', 'c']]
        fig = image_grid_matplotlib(imgs, plot_labels=labels)
        assert isinstance(fig, Figure)

    def test_return_axes(self):
        imgs = self._rand_imgs(2, 2)
        fig, axes, pos = image_grid_matplotlib(imgs, return_axes=True)
        assert isinstance(fig, Figure)
        assert len(axes) == 4

    def test_xy_order(self):
        imgs = self._rand_imgs(2, 3)
        fig = image_grid_matplotlib(imgs, order='xy')
        assert isinstance(fig, Figure)

    def test_outline(self):
        imgs = self._rand_imgs(1, 2)
        fig = image_grid_matplotlib(imgs, outline=True, outline_color=[1, 0, 0])
        assert isinstance(fig, Figure)
