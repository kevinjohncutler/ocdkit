"""Tests for ocdkit.registration — phase cross-correlation and shift stacking."""
...
class TestCrossReg:
    def test_constant_shift(self):
        ...
        stack = np.stack([
            im_shift(base, shift=(0, t * dx), order=1, mode="nearest")
    ...
    def test_moving_reference_max_shift_clips(self):
        shape = (48, 48)
        nframes = 4
        dx = 5
...
class TestPhaseCrossCorrelationGPU:
    def test_basic(self):
        device = torch.device("cpu")
        base = _circle_image((32, 32), center=(16, 12), radius=4)
        stack = torch.from_numpy(np.stack([
            base,
            im_shift(base, shift=(0, 3), order=1, mode="nearest"),
            im_shift(base, shift=(0, 6), order=1, mode="nearest"),
        ], axis=0)).to(device)
...
class TestApplyShifts:
    def test_grouping_and_single(self):
        device = torch.device("cpu")
