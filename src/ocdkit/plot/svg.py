...
# SMPTE 2084 (PQ) constants — same as hostpkg.io.hdr (kept inline here so
# ocdkit doesn't depend on hostpkg).
...
def _p3_linear_to_pq_hdr_uint16(rgb_p3_linear: 'np.ndarray',
    ...
    gamut-mapping. The :func:`hostpkg.io.hdr.encode_rgb_hdr` path uses
...
def jxl_data_url(rgba_arr, effort: int = 1, lossless: bool = True,
    ...
    * ``color='rec2020-pq'`` + ``intensity_target=`` nits — full PQ HDR;
      the data should be a 16-bit array. See ``hostpkg.io.hdr`` for the
...
class SVG:
    ...
    def image(self, x, y, w, h, rgba_arr, *,
        ...
        * ``'jxl-hdr-pq'`` — for **natively HDR-source data only**:
          ...
          (e.g. ``hostpkg.io.hdr.compute_rgb_p3_linear(scene)`` output).
