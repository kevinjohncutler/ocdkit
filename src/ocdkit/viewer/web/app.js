const CONFIG = window.__VIEWER_CONFIG__ || {};
...
// --- Cached canvas bounding rect (issue #4) ---------------------------------
// getBoundingClientRect() forces sync layout. Caching it for the duration of
// one animation frame and listening for invalidating events (resize, scroll,
// canvas size change) cuts hundreds of forced layouts per second of brushing.
let _canvasRectCache = null;
let _canvasRectInvalidationScheduled = false;
function _invalidateCanvasRect() { _canvasRectCache = null; }
function getCanvasRect() {
  if (_canvasRectCache) return _canvasRectCache;
  if (!canvas) return null;
  _canvasRectCache = canvas.getBoundingClientRect();
  if (!_canvasRectInvalidationScheduled) {
    _canvasRectInvalidationScheduled = true;
    requestAnimationFrame(() => {
      _canvasRectInvalidationScheduled = false;
      _canvasRectCache = null;
    });
  }
  return _canvasRectCache;
}
window.addEventListener('resize', _invalidateCanvasRect, { passive: true });
window.addEventListener('scroll', _invalidateCanvasRect, { passive: true, capture: true });
window.addEventListener('orientationchange', _invalidateCanvasRect, { passive: true });
// Some sidebar/panel toggles change the layout; the easiest catch-all is to
// observe size changes on the canvas itself.
if (typeof ResizeObserver !== 'undefined' && canvas) {
  try {
    new ResizeObserver(_invalidateCanvasRect).observe(canvas);
  } catch (err) {
    console.warn('[canvas-rect-cache] ResizeObserver setup failed:', err);
  }
}
// ---------------------------------------------------------------------------
...
const ENABLE_WEBGL_LOGGING = Boolean(window.__VIEWER_WEBGL_LOGGING__);
function perfNow() {
...
function logWebgl(kind, data) {
  ...
  try {
    if (typeof window !== 'undefined' && typeof window.__viewerLogPush === 'function') {
      window.__viewerLogPush(kind, data);
...
// WebGL overlay quality controls (configurable via __VIEWER_CONFIG__)
...
const paintingInitOptions = {
  ...
  onMaskBufferReplaced: (next) => {
    ...
    const lastBuild = typeof window !== 'undefined' && window.__VIEWER_DEBUG__
      ? window.__VIEWER_DEBUG__.lastLocalAffinityBuild
      : null;
...
function logDebugGridStatus(reason) {
  if (!window.__VIEWER_DEBUG__ || !window.__VIEWER_DEBUG__.gridLogs) {
    return;
  }
  ...
  try {
    ...
    console.log('[debug-grid]', reason, {
      ...
      affinityInfo: typeof window.__VIEWER_DEBUG__ === 'object' && window.__VIEWER_DEBUG__
        && typeof window.__VIEWER_DEBUG__.getAffinityInfo === 'function'
        ? window.__VIEWER_DEBUG__.getAffinityInfo()
        : null,
    });
  } catch (err) {
...
function ensureDebugGridBootstrap() {
  ...
  const execute = () => {
    ...
    if (window.__VIEWER_DEBUG__) {
      window.__VIEWER_DEBUG__.gridBootstrapCompleted = true;
    }
  };
...
if (typeof window !== 'undefined') {
  ...
  window.__VIEWER_DEBUG__.walkGridFill = async function walkGridFill(options = {}) {
    ...
    const resolvedTargetLabel = Number.isFinite(options.targetLabel)
      ...
      : ((window.__VIEWER_DEBUG__ && typeof window.__VIEWER_DEBUG__.gridBackgroundLabel === 'number')
        ? window.__VIEWER_DEBUG__.gridBackgroundLabel | 0
        : 0);
...
function rebuildLocalAffinityGraph() {
  ...
  buildAffinityGraphSegments();
  rebuildOutlineFromAffinity();
  if (typeof window !== 'undefined' && window.__VIEWER_DEBUG__) {
    window.__VIEWER_DEBUG__.lastLocalAffinityBuild = {
...
function recomputeNColorFromCurrentMask(forceActive = false) {
  return new Promise((resolve) => {
    try {
      ...
      // Use a view, not a deep copy — base64FromUint8 reads sequentially
      // and synchronously, so the underlying WASM heap can't mutate mid-encode.
      // (issue #6: avoid 16MB+ allocation per recolor on 2K masks.)
      const bytes = new Uint8Array(maskValues.buffer, maskValues.byteOffset, maskValues.byteLength);
      ...
      if (typeof fetch === 'function') {
        fetch('/api/ncolor_from_mask', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
          ...
          .then(applyMapping)
          .catch((err) => { console.warn('[ncolor_from_mask] fetch failed:', err); resolve(false); });
        return;
      }
      resolve(false);
    } catch (err) {
      console.warn('[ncolor_from_mask] unexpected error:', err);
      resolve(false);
    }
  });
}
...
async function relabelFromAffinity() {
  ...
  try {
    ...
    const sourceMask = (nColorActive && nColorInstanceMask && nColorInstanceMask.length === maskValues.length)
      ? nColorInstanceMask
      : maskValues;
    // View, not deep copy (issue #6).
    const bytes = new Uint8Array(sourceMask.buffer, sourceMask.byteOffset, sourceMask.byteLength);
    const b64 = base64FromUint8(bytes);
  ...
  return new Promise((resolve) => {
    try {
      ...
      if (window.pywebview && window.pywebview.api && typeof window.pywebview.api.format_labels === 'function') {
        window.pywebview.api.format_labels(payload).then(applyMapping).catch((err) => {
          console.warn('[format_labels] pywebview call failed:', err);
          resolve(false);
        });
        return;
      }
      if (typeof fetch === 'function') {
        fetch('/api/format_labels', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
          .then((res) => (res.ok ? res.json() : Promise.reject(new Error('HTTP ' + res.status))))
          .then(applyMapping)
          .catch((err) => { console.warn('[format_labels] fetch failed:', err); resolve(false); });
        return;
      }
      resolve(false);
    } catch (err) {
      console.warn('[format_labels] unexpected error:', err);
      resolve(false);
    }
  });
}
...
async function requestMaskRebuild() {
  ...
  // AbortController + timeout via shared helper (issue #5).
  let response = await _fetchSegmentationEndpoint('/api/resegment', payload);
  if (!response.ok) {
    if (response.status === 404) {
      response = await _fetchSegmentationEndpoint('/api/segment', payload);
    }
    if (!response.ok) {
...
// Track the in-flight segmentation request so the UI can abort it (issue #5).
const SEGMENTATION_TIMEOUT_MS = 5 * 60 * 1000;  // 5 minutes
let _segmentationController = null;
function cancelSegmentation() {
  if (_segmentationController) {
    try { _segmentationController.abort(); } catch (_) { /* fine */ }
    _segmentationController = null;
  }
}
window.cancelSegmentation = cancelSegmentation;

async function _fetchSegmentationEndpoint(url, payload) {
  cancelSegmentation();  // never let two requests run concurrently
  const controller = new AbortController();
  _segmentationController = controller;
  const timeoutId = setTimeout(() => {
    try { controller.abort(); } catch (_) { /* fine */ }
  ...
  try {
    ...
    if (_segmentationController === controller) {
      _segmentationController = null;
    }
  }
}
...
async function requestSegmentation() {
  ...
  const response = await _fetchSegmentationEndpoint('/api/segment', settings);
  if (!response.ok) {
    throw new Error('HTTP ' + response.status);
  }
  return response.json();
...
if (typeof window !== 'undefined') {
  window.__VIEWER_DEBUG__ = window.__VIEWER_DEBUG__ || {};
  window.__VIEWER_DEBUG__.getCounters = () => ({
  ...
  window.__VIEWER_DEBUG__.resetCounters = () => {
  ...
  window.__VIEWER_DEBUG__.initializeWebglPipelineResources = (imageSource = null) => {
    initializeWebglPipelineResources(imageSource);
  };
  window.__VIEWER_DEBUG__.markMaskTextureFullDirty = () => {
    markMaskTextureFullDirty();
  };
  window.__VIEWER_DEBUG__.markOutlineTextureFullDirty = () => {
    markOutlineTextureFullDirty();
  };
  window.__VIEWER_DEBUG__.applyMaskRedrawImmediate = () => {
    applyMaskRedrawImmediate();
  };
  window.__VIEWER_DEBUG__.requestPaintFrame = () => {
    requestPaintFrame();
  };
  window.__VIEWER_DEBUG__.draw = () => {
    draw();
  };
  window.__VIEWER_DEBUG__.isWebglPipelineActive = () => isWebglPipelineActive();
  window.__VIEWER_DEBUG__.getMaskUploadState = () => ({
  ...
  window.__VIEWER_DEBUG__.collectFillDiagnostics = (index = null) => {
    ...
    paintingDiagnostics: {
      missingMaskDirty: (typeof globalThis === 'object' && globalThis.__VIEWER_DEBUG__)
        ? (globalThis.__VIEWER_DEBUG__.missingMaskDirty || 0)
        : 0,
      missingOutlineDirty: (typeof globalThis === 'object' && globalThis.__VIEWER_DEBUG__)
        ? (globalThis.__VIEWER_DEBUG__.missingOutlineDirty || 0)
        : 0,
    },
      finalizeCallCount: state && typeof state.finalizeCallCount === 'number'
        ? state.finalizeCallCount
        : 0,
      finalizeHistory: (typeof globalThis === 'object' && globalThis.__VIEWER_DEBUG__ && globalThis.__VIEWER_DEBUG__.finalizeHistory)
        ? globalThis.__VIEWER_DEBUG__.finalizeHistory.slice(-8)
        : null,
  };
    console.log('[debug-fill-report]', report);
    return report;
  };
  window.__VIEWER_DEBUG__.summarizeDirtyRegions = () => ({
    maskDirtyRegionCount: maskDirtyRegions.length,
  ...
  window.__VIEWER_DEBUG__.getDirtyTracker = () => Object.assign({}, DEBUG_DIRTY_TRACKER);
  window.__VIEWER_DEBUG__.summarizeAffinityGraph = () => {
    if (!affinityGraphInfo || !affinityGraphInfo.values) {
      return {
        hasGraph: false,
        showAffinityGraph,
  ...
  window.__VIEWER_DEBUG__.highlightFillRect = (rect) => {
    showDebugFillOverlay(rect);
  };
  window.__VIEWER_DEBUG__.hashColor = (label) => {
  ...
  window.__VIEWER_DEBUG__.resetWebglPipeline = () => {
    if (webglPipeline && webglPipeline.gl) {
      try {
  ...
  window.__VIEWER_DEBUG__.getAffinityInfo = () => ({
    ...
    showAffinityGraph,
    lastLocalAffinityBuild: window.__VIEWER_DEBUG__.lastLocalAffinityBuild || null,
  });
  window.__VIEWER_DEBUG__.setAffinitySentinel = (value = 255) => {
  ...
  window.__VIEWER_DEBUG__.getAffinityPersistSnapshot = () => ({
    source: affinityGraphSource,
  ...
  window.__VIEWER_DEBUG__.getAffinityState = () => ({
    showAffinityGraph,
    affinitySegEnabled,
    affinityGraphSource,
  ...
  window.__VIEWER_DEBUG__.getMaskValues = () => maskValues;
  // Debug function to access nColorInstanceMask
  window.__VIEWER_DEBUG__.getNColorInstanceMask = () => nColorInstanceMask;
  // Debug function to check nColor state
  window.__VIEWER_DEBUG__.getNColorState = () => ({
    nColorActive,
  ...
  window.__VIEWER_DEBUG__.updateAffinityForIndices = (indices) => {
    if (!indices || !indices.length) return { error: 'no indices' };
    const before = window.__VIEWER_DEBUG__.getAffinityState();
    updateAffinityGraphForIndices(indices);
    const after = window.__VIEWER_DEBUG__.getAffinityState();
    return { before, after, indicesCount: indices.length };
  };
  // Debug function to check affinity edges in a region
  window.__VIEWER_DEBUG__.checkAffinityInRegion = (x, y, w, h) => {
    if (!affinityGraphInfo || !affinityGraphInfo.values) {
  ...
  window.__VIEWER_DEBUG__.paintAndUpdateAffinity = (x, y, w, h, label) => {
    ...
    const stateBefore = window.__VIEWER_DEBUG__.getAffinityState();
    updateAffinityGraphForIndices(indices);
    const stateAfter = window.__VIEWER_DEBUG__.getAffinityState();
    const affinityInRegion = window.__VIEWER_DEBUG__.checkAffinityInRegion(x, y, w, h);
    return {
