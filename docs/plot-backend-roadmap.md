...
**Driver:** matplotlib accounts for ~80% of warm-call time in `hostpkg.plot.plot_spectra` (~115 ms of ~140 ms) and ~1.5 s of cold-import overhead. We also want native hover/tooltip behavior like the classification-debugger GUI, which matplotlib can't deliver inline.

## Context

`ocdkit.plot` and `hostpkg.plot` already own ~10 K LOC of plotting code:

| Package | LOC |
|---|---|
| `ocdkit.plot` (figure, grid, label, color, defaults, display, contour, export) | 1,935 |
| `hostpkg.plot` non-WGPU (datashade, cell, text, barcode, line, background, …) | 4,641 |
| `hostpkg.plot.wgpu` (lines, scatter, aggregators — already custom GPU primitives) | 4,783 |
...
| pygfx | 84 MB | 20 K | None — uses `wgpu-py` | Same backend ocdkit/hostpkg already use |
...
Estimated scope, building on existing `hostpkg.plot.wgpu.lines`:
...
| `hostpkg.plot.wgpu.text` | 600 | FreeType-rendered glyph atlas + WGSL textured-quad shader. The one piece of genuinely new rasterization work |
...
Plus: deletion of matplotlib-specific code paths in `hostpkg.plot` (~1 K LOC saved) and removal of matplotlib runtime dep.
...
Spike `plot_spectra_pygfx` using `scope.mixed_spectra[-1]` from `notebooks/hostpkg_demo_notebook.ipynb` as the test case. Compare visual output side-by-side with `plot_spectra_wgpu` and `plot_spectra_cpu`. Decide pygfx-vs-roll-own from that single comparison.
